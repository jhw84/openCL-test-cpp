...
int B::sumTwoValuesOnGPU(int in1, int in2)
{
    ...
    cl::Buffer buffer_A(context,CL_MEM_READ_WRITE,sizeof(int)*1);
    cl::Buffer buffer_B(context,CL_MEM_READ_WRITE,sizeof(int)*1);
    cl::Buffer buffer_C(context,CL_MEM_READ_WRITE,sizeof(int)*1);
    ...
    queue.enqueueWriteBuffer(buffer_A,CL_TRUE,0,sizeof(int)*1,A);
    ...
    queue.enqueueReadBuffer(buffer_C,CL_TRUE,0,sizeof(int)*1,C);
    return C[0];
}
